...
class "CTalentTree"
{
	__init = function(self)
		...
		if (ModernTalents_DB and ModernTalents_DB.position) then
			local pos = ModernTalents_DB.position
			self.frame:ClearAllPoints()
			self.frame:SetPoint(pos.point, UIParent, pos.relPoint, pos.x, pos.y)
		end
		if (ModernTalents_DB and ModernTalents_DB.scale) then
			self.frame:SetScale(ModernTalents_DB.scale)
		end
	...
	BuildSpecs = function(self)
		...
		titleIcon:SetTexture("Interface\\AddOns\\TeronModernTalents\\Assets\\Talents\\classicon-" .. string.lower(englishClass))
		...
		for t = 1, numTabs do
			...
			local bgBase = "Interface\\AddOns\\TeronModernTalents\\Assets\\Talents\\Backgrounds\\talentbg-" .. string.lower(englishClass) .. "-" .. t
	...
	Refresh = function(self)
		...
		-- file-load time, before ModernTalents_DB's SavedVariables are actually injected (same
